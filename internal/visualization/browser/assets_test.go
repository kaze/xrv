...
func TestLoadAssets_Production(t *testing.T) {
	fs, err := LoadAssets()
	if err != nil {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if err != nil {
				t.Errorf("Expected %s to be accessible, got error: %v", tc.path, err)
				return
			}
			defer file.Close()
...
func TestGetTemplates_Production(t *testing.T) {
